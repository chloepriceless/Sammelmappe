...
- **🧾 Liste mit Live-Summe** — markiere beliebige offene Belege (oder per Button **alle offenen auf einmal**), die Summe wird oben live mitgeführt. Ein **„Öffnen"-Button** pro Beleg öffnet das Detail mit einem einzigen Klick.
...
| `CSRF_TRUSTED_ORIGINS`     | _(leer)_                         | Nur für exotische Proxy-Setups: komma-getrennte zusätzliche Hostnamen oder volle Origins (z.B. `app.example` oder `https://app.example`), von denen mutierende Browser-Requests kommen dürfen. Normal leer — der CSRF-Schutz akzeptiert Origins, die zum `Host` (bzw. `X-Forwarded-Host` vom in `TRUSTED_PROXIES` eingetragenen Proxy) passen. Geblockte Requests stehen mit Details im Log. |
| `CSRF_STRICT`              | `false`                          | Streng: blockt mutierende Requests, die weder `Sec-Fetch-Site` noch `Origin` tragen. Default aus, weil das API-Scripting (curl) bricht; einschalten, um auch das Alt-Browser-Restrisiko zu schließen. |
